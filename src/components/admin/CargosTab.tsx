...
type Cargo = { id: string; nome: string; multiplicador: number; cor: string; ordem: number; setor_id: string | null };
const blank = (ordem: number): Omit<Cargo, 'id'> => ({ nome: '', multiplicador: 1.0, cor: '#6366f1', ordem, setor_id: null });
...
export function CargosTab() {
  ...
  const openEdit = (c: Cargo) => {
    setEditingId(c.id);
    setForm({ nome: c.nome, multiplicador: c.multiplicador, cor: c.cor || '#6366f1', ordem: c.ordem, setor_id: c.setor_id });
    setOpen(true);
  };

  const save = async () => {
    if (!form.nome.trim()) return toast({ title: 'Preencha o nome do cargo', variant: 'destructive' });
    const payload = { ...form, multiplicador: Number(form.multiplicador), setor_id: form.setor_id || null };
                ...
                {setor ? (
                  <>
                    <span
                      ...
                      style={{ background: setor.cor || '#888' }}
                    />
                      ...
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-2">
                          <span
                            ...
                            style={{ background: c.cor || setor?.cor || '#888' }}
                          />
