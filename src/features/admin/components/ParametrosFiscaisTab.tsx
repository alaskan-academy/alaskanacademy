...
export function ParametrosFiscaisTab() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const rows = r2.data || [];
      setFatBruto(  rows.reduce((s: number, r: any) => s + Number(r.faturamento_bruto  || 0), 0));
      setTaxaPlat(  rows.reduce((s: number, r: any) => s + Number(r.taxa_plataforma    || 0), 0));
      setInvestMeta(rows.reduce((s: number, r: any) => s + Number(r.investimento_meta  || 0), 0));
      setReembolsos(rows.reduce((s: number, r: any) => s + Number(r.reembolsos         || 0), 0));
      setLoading(false);
    };
    load();
  }, []);
  ...
  const fields = [
    { key: 'imposto_simples_nacional_pct' as const, label: 'Imposto Simples Nacional (%)', step: 0.01 },
    { key: 'imposto_meta_ads_pct'         as const, label: 'Imposto Meta Ads (%)',         step: 0.01 },
    { key: 'custo_fixo_mensal'            as const, label: 'Custo Fixo Mensal (R$)',        step: 1    },
  ];
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-3xl">
      <div className="bg-card border border-border rounded-lg p-6 space-y-4">
        <div>
          <h3 className="text-sm font-medium text-foreground mb-1">Parâmetros fiscais</h3>
          <p className="text-xs text-muted-foreground">
            A taxa da plataforma Payt é calculada automaticamente e não é configurável aqui.
          </p>
        </div>
        ...
        <Button onClick={handleSave} className="w-full mt-2">Salvar Configurações</Button>
      </div>
