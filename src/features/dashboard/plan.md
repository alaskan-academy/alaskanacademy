...
- [x] **O webhook da Payt não entrega upsells — descoberto pela conciliação com o export.** Comparando o export de 01–20/08 com o banco: 1.358 pedidos aprovados na Payt contra 1.313 no banco. Os 45 ausentes faltam também em `vendas_payt`, a camada bruta, então o evento nunca chegou. Todos têm `tipo_venda = Upsell` e nome de checkout vazio. Venda direta: 1.312 na Payt, 1.312 no banco — entrega perfeita. Upsell: 46 na Payt, 0 no banco. São R$ 10.220,70 no período, principalmente Handify Artesanato Completo (32 × R$ 297) e Saboaria Energética (11 × R$ 18,70)
- [ ] **Configurar o webhook da Payt para disparar em upsell** — ação no painel, não no código. Enquanto não for feito, toda a receita de upsell fica fora do dashboard
- [ ] **A detecção de upsell por sessão está medindo outra coisa.** Ela achou 26 "upsells" por segunda compra em 30 min, mas os upsells reais nunca chegam ao banco. Aqueles 26 são outro fenômeno — provavelmente compra dupla legítima. Quando o webhook passar a entregar upsells, `tipo_venda` vira o sinal correto e a heurística deve ser aposentada
